...
def get_outstanding(bpf, pid, min_age_ns, top):
    ...
    return list(
        map(
            lambda alloc: {
                "stack": [s.decode("ascii") for s in alloc.stack],
                "size": alloc.size,
                "count": alloc.count,
            },
            sorted_stacks,
        ...
        entries.append(
            {
                ...
                "trace": get_trace_info_as_list(
                    bpf, pid, stack_traces, allocation.stack_id.value
                ),
            }
        )
    return {
...
def _get_additional_syscall_info(bpf, pid):
    ...
    for k, v in syscall_counts_stacks.items():
        ...
        stack_trace = "\n\t".join(
            get_trace_info_as_list(bpf, pid, bpf["stack_traces"], v.stack_id)
        )
...
def save_snapshot(
    ...
    with open(
            get_result_file_name(snapshot_dir, snapshot_prefix or "snapshot", "json"), "w"
    ) as outfile:
        json.dump(snapshot, outfile, indent=2, cls=SetEncoder)
...
def attach_probes(
    ...
    bpf = BPF(
        ...
        cflags=[
            ...
            # "-DDEBUG",  # TODO: move to global variable and add checks
        ],
    )
    ...
    while not process.isalive():
        ...
        if process.terminated:
            raise Exception(
                f"Process is already terminated, with status code {process.exitstatus}"
            )
    ...
    bpf.attach_uprobe(
        name=runner, sym_re=".*ucsb.*timer_t.*start.*", fn_name="bench_enter"
    )
    bpf.attach_uprobe(
        name=runner, sym_re=".*ucsb.*timer_t.*pause.*", fn_name="bench_exit"
    )
    bpf.attach_uprobe(
        name=runner, sym_re=".*ucsb.*timer_t.*resume.*", fn_name="bench_enter"
    )
    bpf.attach_uprobe(
        name=runner, sym_re=".*ucsb.*timer_t.*stop.*", fn_name="bench_exit"
    )
...
def signal_handler(_sig_no, _stack_frame):
    ...
    # print("Took", time.time_ns() - start_time, "ns")
...
signal.signal(signal.SIGUSR1, lambda _, __: set_start_time())
...
def sleep_and_check(
        interval: float, pid: int, process: Optional[spawn], check_delay: float
):
    wait_til = time.time() + interval
    while time.time() < wait_til:
